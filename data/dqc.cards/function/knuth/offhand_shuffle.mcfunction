...
execute store result storage dqc.cards:tmp count int 0.99 run data get storage dqc.cards:tmp deck

# Check if we have enough elements to shuffle, then shuffle recursively
execute if predicate {"condition":"minecraft:value_check","value":{"type":"minecraft:storage","storage":"dqc.cards:tmp","path":"count"},"range":{"min":1,"max":53}} run function dqc.cards:knuth/shuffle_loop
